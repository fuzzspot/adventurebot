        ...
        paths.forEach((path: string) => {
            fs.readFile(path, 'utf8', function (err, data) {
                ...
                if (data.length > 0 && data != null) {
                    var scriptMatcher = new RegExp(/(?<=\[script]\n)(.*)(?:=\n\[Options|\n\n)/ms)
                    var textMatcher = new RegExp(/(?<=\[Text]\n)(.*)(?:=\n\[Options|\n\n)/ms)
                    var optionMatcher = new RegExp(/(?<=\[Options]\n)(.*)(\n)/ms)

                    var scriptGroup = data.match(scriptMatcher)
                    ...
                    if (scriptGroup != null) {
                        script = scriptGroup[0]
                    }
                    if (textGroup != null) {
                        text = textGroup[0]
                        //TO DO rewrite the the parsing of the options.
                        /*
                        * new option group structure:
                        * {
                        * optionsubgroups; null if there is no subgroup inside;
                        * options;
                        * optionsvisibilityscript;
                        * optionpostselectionscript;
                        * }
                        **/
                        let raw = path.split(/\/(?=[^/]+$)/)
                        ...
                        insertStory(`${author}/${story}/${page}`, `${author}/${story}`, page, script, text, options).catch(e => {
                            logger.log('error', e.message, ...[e])
                        })
                    }
                }
