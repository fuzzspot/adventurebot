...
export default async function execute(msg: Message, data: string): Promise<void> {
    try {
        ...
        if (objectMatch == null) {
            ...
            loadPage(options[objectMatch]).then((doc: { _id: string, story: string, page: string,script:string, text: string, options: object }) => {
                const name = doc.story
                const options: any[] = []
                //TO DO parse and execute the script;
                //TO DO replace variables in text with corresponding value
                //TO DO evaluate what options are unlocked
                //TO DO alter the options visualiation
